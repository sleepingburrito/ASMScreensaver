...
void Start()
{
	...
	//seed random values
	SetInst( startaddr++, SET_REG_VALUE_FULL, 0x0, 0 ); 
	SetAddrs( startaddr++, 0x0A0F );

	SetInst( startaddr++, SET_REG_VALUE_FULL, 0x1, 0 );
	SetAddrs( startaddr++, 0x100F );

	//start location to dump data
	SetInst( startaddr++, SET_REG_VALUE_FULL, 0x3, 0 );
	SetAddrs( startaddr++, 0x01F0 );

	//end location
	SetInst( startaddr++, SET_REG_VALUE_FULL, 0x4, 0 );
	SetAddrs( startaddr++, 0xF1F0 );

	//mixer
	uint16_t startOfMixer = startaddr;

	//attemp at random values
	SetInst( startaddr++, ADD_REG, 0x2, 0x1 );
	SetInst( startaddr++, MULT_REG, 0x2, 0x0 );
	SetInst( startaddr++, INC_REG, 0x0, 0x0 );

	//dump data to ram
	SetInst( startaddr++, COPY_TO_AD_REG, 0x2, 0x3 );
	SetInst( startaddr++, INC_REG, 0x3, 0x0 );

	//jump if not at end yet
	SetInst( startaddr++, JMP_IF_LT, 0x3, 0x4 );
	SetAddrs( startaddr++, startOfMixer );

	//reset dump location back to top
	SetInst( startaddr++, SET_REG_VALUE_FULL, 0x3, 0 );
	SetAddrs( startaddr++, 0x01F0 );
